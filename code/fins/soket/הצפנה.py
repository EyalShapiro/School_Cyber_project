...
def Deciphering_File_wav(name_file):  # file wav RSA
    ...
    with open(locate+"decrypted_"+name_file, 'wb+') as decrypted_file:
...
def Encryption_File_wav(name_file):  # file wav RSA
    # מצפין את תןכן הקובץ
    l = name_file.split('.')
    if l[-1] != 'wav':
        return 'This is Not a correct file'
...
def main():
    ...
    file = "hello.wav"
    print('File is encrypted⇨ ', Encryption_File_wav(file), '\n')
    print('File is decrypted⇨ ', Deciphering_File_wav(file), '\n')
    # text = 'Hello and help'
